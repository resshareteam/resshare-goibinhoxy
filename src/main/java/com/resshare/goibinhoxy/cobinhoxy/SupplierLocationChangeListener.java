...
public class SupplierLocationChangeListener extends ListenerBase {

	@Override
	public void onChildAdded(DataSnapshot snapshot1, String previousChildName) {
		try {
			if (snapshot1.child("processing").getValue() == null) {
				  String user_id_supplier = snapshot1.child("user_id").getValue(String.class);
				...
				FirebaseDatabase.getInstance()
						.getReference(ResFirebaseReference.getDataPathReference(
								FirebaseRefOxy.order + "/" + order_key + "/supplier_info/current"))
						.setValue(currentHashMap);

				
				FirebaseDatabase.getInstance()
				.getReference(ResFirebaseReference.getDataPathReference(
						FirebaseRefOxy.order + "/" + order_key+"/customer_info" )).addListenerForSingleValueEvent(new ValueEventListener() {
							...
							public void onDataChange(DataSnapshot snapshot) {
								if(snapshot!=null)
								{
									if(snapshot.exists())
									{
										String user_id_customer = snapshot.child("user_id").getValue(String.class);
										...
										if(event_cus!=null)
										{
											...
											viewHashMap.put("user_id_destination", user_id_customer);
											
											HashMap data = new HashMap<>();
											HashMap script_param = new HashMap<>();
											script_param.put("location", location);
											script_param.put("user_id", user_id_supplier);
	...
	private Object getScript() {
		// TODO Auto-generated method stub
		return   SupplierLocationChangeUI .class.getName();
	}

	public String getReferenceName() {

		// TODO Auto-generated method stub
		return ResFirebaseReference.getInputPathReference(FirebaseRefOxy.supplier_location_changed);
	}
